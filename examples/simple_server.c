...
static void on_server_close(uv_handle_t* handle) {
    ...
    app_context_t* ctx = (app_context_t*)uvhttp_server_get_user_data(server);
    ctx->server_closed = 1;
    // Stop the loop only when both server and signal handles are closed
    if (ctx->signal_closed) {
        uv_stop(uvhttp_server_loop(ctx->server));
    }
}
...
int main(int argc, char *argv[]) {
    ...
    uvhttp_server_set_user_data(app_ctx.server, &app_ctx);

    if (uvhttp_server_listen(app_ctx.server) != 0) {
    ...
    uvhttp_server_destroy(app_ctx.server);
